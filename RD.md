...
* **迪米特法則** : 
- 核心定義：又稱「最少知識原則」（Least Knowledge Principle），強調一個物件應對其他物件保持最少的瞭解。
- 溝通對象限制：一個物件只應與其「直接的朋友」交談，不與「陌生人」說話。
- 主要效益：降低類別之間的耦合度（Coupling），提高代碼的可維護性與可測試性。
- 負面影響：過度使用會導致系統中出現大量的中繼方法（Wrapper methods），增加呼叫鏈的複雜度。

* **DTO/REPO/SERVICE** : DTO/REPO/SERVICE 層級 區別分明
